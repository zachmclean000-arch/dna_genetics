...
const records=JSON.parse(fs.readFileSync('reference/regular-product-records.json','utf8'));
if(records.length!==14)throw Error(`Expected 14 inspected products, received ${records.length}`);
...
try {
 ...
 await transaction(s=>{
  for(const record of records){
   ...
   if(existing?.sourceUrl && existing.category !== 'Regular Seeds') {
     existing.additionalCategories=[...new Set([...(existing.additionalCategories||[]),'Regular Seeds'])];
     continue;
   }
 ...
 const products=await prisma.product.findMany({where:{sourceUrl:{not:null},OR:[{category:{name:"Regular Seeds"}},{additionalCategories:{some:{name:"Regular Seeds"}}}]},include:{variants:true}});
