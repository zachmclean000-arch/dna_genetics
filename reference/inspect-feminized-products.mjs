...
try {
 ...
 for(let number=1;number<=3;number++){
  const url='https://dnagenetics.com/product-category/feminized-seeds/'+(number===1?'':`page/${number}/`)+'?orderby=date';
 ...
 for (const slug of ["gmo-kosher", "snack-pack", "white-chocolope"]) urls.add(`https://dnagenetics.com/product/${slug}/`);
 ...
 for(const url of urls){
  ...
  records.push(record);fs.writeFileSync('reference/feminized-product-records.json',JSON.stringify(records,null,2));
...
fs.writeFileSync("reference/feminized-product-records.json",JSON.stringify(records,null,2));
