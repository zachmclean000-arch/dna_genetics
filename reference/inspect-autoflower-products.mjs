...
try {
 ...
 for(let number=1;number<=2;number++){
  const url='https://dnagenetics.com/product-category/autoflower-seeds/'+(number===1?'':`page/${number}/`)+'?orderby=date';
 ...
 for(const url of urls){
  ...
  records.push(record);fs.writeFileSync('reference/autoflower-product-records.json',JSON.stringify(records,null,2));
...
fs.writeFileSync("reference/autoflower-product-records.json",JSON.stringify(records,null,2));
