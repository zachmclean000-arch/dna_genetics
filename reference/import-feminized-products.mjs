...
const records=JSON.parse(fs.readFileSync('reference/feminized-product-records.json','utf8'));
if(records.length!==42)throw Error(`Expected 42 inspected products, received ${records.length}`);
...
try {
 ...
 const products=await prisma.product.findMany({where:{sourceUrl:{not:null}},include:{variants:true}});
